...
TEST_NODE_IP="10.0.0.1"  # 修改为你的测试设备的实际 IP 地址
TEST_NODE_PWD="your_password"  # 修改为你的测试设备的实际密码

#Configuring Test Device Information
devices_json='{
  "ap22280960d47301729": "/dev/device,/dev/powerRelay_for_bluetooth1",
}'# 根据实际设备更新
...
#Result comparison
#主要目的是对测试结果进行比较，确保新测试运行的结果与之前的结果一致或有预期的变化
...
#为测试结果和报告创建一个远程存储路径，并确保能够将测试结果上传到指定的服务器上
#cd $WORKSPACE
