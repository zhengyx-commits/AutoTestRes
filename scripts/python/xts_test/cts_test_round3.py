...
run_command = '/home/ubuntu2004/Desktop/android-cts-12/android-cts/tools/cts-tradefed run retry --retry {} --retry-type failed --shard-count 6 -s ohm0000000031 -s ohm0000000035 -s ohm0000000032 -s ohm0000000033 -s ohm0000000034 -s ohm0000000036\n'
run_command2 = '/home/ubuntu2004/Desktop/android-cts-12/android-cts/tools/cts-tradefed run retry --retry {} -s ohm0000000036\n'
# run_command3 = '/home/ubuntu2004/Desktop/android-cts-12/android-cts/tools/cts-tradefed run retry --retry {} -s ohm0000000032\n'
...
if __name__ == '__main__':
    ...
    # result_info = subprocess.check_output('/home/ubuntu2004/Desktop/android-cts-12/android-cts/tools/cts-tradefed l r'.split(),encoding='utf-8')
    # print(result_info)
    # last_report_num = count_last_result_num(result_info)
    # print(last_report_num)
    # run1 = run_remote_command(channel, run_command.format(last_report_num))
    # time.sleep(180)
    # channel.close()
    ...
    while count < MAX_RETRY:
        ...
        if last_report_failed == 0:
            ...
            if last_report_failed > 100:
                retry = run_remote_command(channel, run_command.format(last_report_num))
            else:
                os.system("adb -s ohm0000000036 shell cmd wifi connect-network Asus_xts_5G-2 wpa2 XPxp5lxts")
                time.sleep(10)
                retry = run_remote_command(channel, run_command2.format(last_report_num))
            time.sleep(180)
            channel.close()
            count += 1
    ssh.close()
