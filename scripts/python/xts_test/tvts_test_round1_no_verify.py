...
run_command = '/home/amlogic/Desktop/android-tvts/tools/tvts-tradefed run tvts-cert --shard-count 3 -s ' \
              'oppen000643 -s oppen000645 -s oppen000655\n'
print(run_command)
retry_command_regu1 = '/home/amlogic/Desktop/android-tvts/tools/tvts-tradefed run commandAndExit retry -r {} ' \
                      '--shard-count 3 -s oppen000643 -s oppen000645 -s oppen000655\n'
retry_command_regu2 = '/home/amlogic/Desktop/android-tvts/tools/tvts-tradefed run commandAndExit retry -r {} -s ' \
                      'oppen000643'
fail_list = []
last_fail_num = 0
MAX_RETRY = 5
...
if __name__ == '__main__':
    ...
    while count <= MAX_RETRY:
        ...
        result_info = subprocess.check_output('/home/amlogic/Desktop/android-tvts/tools/tvts-tradefed l r'.split(),
                                              encoding='utf-8')
        print(result_info)
        ...
        if last_report_failed == 0:
            ...
            if count <= 3:
                retry = run_remote_command(channel, retry_command_regu1.format(last_report_num))
            else:
                retry = run_remote_command(channel, retry_command_regu2.format(last_report_num))
        time.sleep(600)
        channel.close()
        count += 1
    ssh.close()
