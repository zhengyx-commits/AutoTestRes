...
video, play_command = amplayer_play_cmd(p_conf_amplayer_path['EVO_path'])
...
@pytest.mark.skipif(android_version == "34", reason="Android U is not support evo format")
def test_amplayer_play_evo():
    assert p_conf_uuid in localPlayer.getUUIDs()
    print(play_command)
    localPlayer.run_shell_cmd(play_command)
    assert playerCheck.run_check_main_thread(30, type="evo"), "start play failed"
