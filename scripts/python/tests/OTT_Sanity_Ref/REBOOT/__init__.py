...
def get_boot_logo(first_logo=None, second_logo=None, launcher=None):
    logo_find = True
    ref_boot_image = resmanager.get_target("image/boot.png", source_path="image/boot.png")
    ref_google_tv_image = resmanager.get_target("image/google_tv.png", source_path="image/google_tv.png")
    # while True:
    ...
    if first_logo and second_logo:
        ...
        second_process = subprocess.run(["tools/VideoStateDetector", "--method", "2", "--video_path", f"{file}", "--background_image_path", ref_google_tv_image, "--saved_path", f'{obs.screenshot_dir}{timestamp}_second.png', '--match_number', '1', '--cooldown_time', '0'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        logging.info(f"process: {second_process}")
        second_output = eval(second_process.stdout.decode())
        if not (first_output['Matched time'] and second_output['Matched time']):
            logo_find = False
            return logo_find
        else:
            return True
