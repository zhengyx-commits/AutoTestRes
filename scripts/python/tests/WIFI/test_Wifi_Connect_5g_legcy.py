...
# @Time    : 2022/6/16 18:26
# @Author  : chao.li
# @Site    :
# @File    : test_Wifi_Connect_5g_legcy.py
# @Software: PyCharm
...
wifi = WifiTestApk()
p_conf_router_info = yamlTool(os.getcwd() + '/config/config_wifi.yaml').get_note('conf_connect_5g_legcy')
...
def setup():
    ...
    router = Router(band=p_conf_router_info['band'], ssid=p_conf_router_info['ssid'],
                    ...
                    wep_encrypt=p_conf_router_info['wep_encrypt'], wep_passwd=p_conf_router_info['wep_passwd'])
    ac88u.change_setting(router)
    # connect wifi
    connect_wifi(router, type='WEP', passwd=router.wep_passwd)
    yield
    forget_wifi()
