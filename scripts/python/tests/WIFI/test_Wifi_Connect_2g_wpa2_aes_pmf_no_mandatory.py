...
# @Author  : chao.li
# @Site    :
# @File    : test_Wifi_Connect_2g_wpa2_aes_pmf_no_mandatory.py
# @Software: PyCharm
...
'''
测试配置
1.设置路由器2.4G 无线网络名称为“ATC_ASUS_AX88U_2G”，隐藏SSID设置为否，无线模式设置为自动，频道带宽设置为20/40M,信道设置为自动，授权方式为WPA2-Personal,WPA加密选择AES，WPA-PSK无线密码设置为Abc@$123456,受保护的管理帧设置为非强制启用
2.连接2.4G SSID
...
wifi = WifiTestApk()
p_conf_router_info = yamlTool(os.getcwd() + '/config/config_wifi.yaml').get_note('conf_connect_2g_wpa2_aes_pmf_no_mandatory')
...
def test_connect_wpa2_aes_pmf_no_mandatory_ssid():
