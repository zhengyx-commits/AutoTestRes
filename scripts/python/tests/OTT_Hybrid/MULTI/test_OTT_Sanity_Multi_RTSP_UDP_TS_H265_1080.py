...
def test_RTSP_UDP_TS_H265_1080():
    single_stream = p_conf_single_stream
    if single_stream:
        stream_name_list, url = get_conf_url("conf_rtsp_url", "rtsp_TS_H265_1080", "conf_stream_name", "h265_1080P")
        file_path = streamProvider.get_file_path('h265_1080P', 'ts', stream_name_list[0])
        if len(file_path) >= 1:
        ...
        stream_name_list = get_conf_url("conf_rtsp_url", "rtsp_TS_H265_1080")
        for stream_name in stream_name_list:
            ...
            if stream_name:
                ...
                multi.send_cmd(start_cmd)
                assert playerCheck.check_startPlay(protocol="rtsp")[0], "start playback failed"
                multi.stop_multiPlayer_apk()
