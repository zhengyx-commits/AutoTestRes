...
# @Time    : 2021/12/20 15:49
# @Author  : chao.li
# @Site    :
# @File    : test_OTT-Sanity_Program_Scan_T.py.py
# @Software: PyCharm
import logging
import time
...
p_conf_channel_count = p_conf_dvb['DVB-T']['channel_count']
p_conf_frequency_point = p_conf_dvb['DVB-T']['frequency_point']
# logging.info(f'scan_c p_conf_channel_count:{p_conf_channel_count}, p_conf_frequency_point:{p_conf_frequency_point}')


@pytest.fixture(autouse=True)
def setup_teardown():
    dtv.select_type('DVB-T')
    time.sleep(1)
    yield
    dtv.home()
