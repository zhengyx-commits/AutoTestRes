...
p_conf_udp_4kP30_h265 = config_yaml.get_note("kpi_config").get("live_s_t_kpi").get("udp_4kP30_h265")
...
for g_conf_device_id in devices:
    kpianalyze = KpiAnalyze(for_framework=True, adb=g_conf_device_id, config="/config/config.yaml", kpi_name="vod_s_t_kpi",
                            targetfile=None, kpifileobj=f"udp_h265_4kP30_start_{g_conf_device_id}.txt", kpifileobj_xlsx=f"udp_h265_4kP30_start_{g_conf_device_id}.xlsx")
...
def multi_teardown():
    ...
    setprop()
    prepare("conf_udp_url", "udp", "conf_stream_name", "h265_4K")
...
def test_kpi_result():
    ...
    kpianalyze.save_to_db(name='vod_s_t_start_kpi', flag='udp_h265_4kp30')
    save_result(kpianalyze.kpifileobj_xlsx)
    kpianalyze.filter_abnormal_value(p_conf_udp_4kP30_h265)
