...
p_conf_udp_1080I_h264 = config_yaml.get_note("kpi_config").get("live_s_t_kpi").get("udp_1080I_h264")
...
for g_conf_device_id in devices:
    kpianalyze = KpiAnalyze(for_framework=True, adb=g_conf_device_id, config="/config/config.yaml", kpi_name="vod_s_t_kpi",
                            targetfile=None, kpifileobj=f"udp_h264_1080i_start_{g_conf_device_id}.txt", kpifileobj_xlsx=f"udp_h264_1080i_start_{g_conf_device_id}.xlsx")
...
def multi_teardown():
    ...
    setprop()
    prepare("conf_udp_url", "udp", "conf_stream_name", "h264_1080I")
...
def test_kpi_result():
    ...
    kpianalyze.save_to_db(name='vod_s_t_start_kpi', flag='udp_h264_1080i')
    save_result(kpianalyze.kpifileobj_xlsx)
    kpianalyze.filter_abnormal_value(p_conf_udp_1080I_h264)
