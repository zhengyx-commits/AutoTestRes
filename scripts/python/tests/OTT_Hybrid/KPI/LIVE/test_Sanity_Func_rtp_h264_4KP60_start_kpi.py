...
p_conf_rtp_4kP60_h264 = config_yaml.get_note("kpi_config").get("live_s_t_kpi").get("rtp_4kP60_h264")
...
for g_conf_device_id in devices:
    kpianalyze = KpiAnalyze(for_framework=True, adb=g_conf_device_id, config="/config/config.yaml", kpi_name="vod_s_t_kpi",
                            targetfile=None, kpifileobj=f"rtp_h264_4kP60_start_{g_conf_device_id}.txt", kpifileobj_xlsx=f"rtp_h264_4kP60_start_{g_conf_device_id}.xlsx")
...
def multi_teardown():
    ...
    setprop()
    prepare("conf_rtp_url", "rtp", "conf_stream_name", "h264_4K_1")
...
def test_kpi_result():
    ...
    kpianalyze.save_to_db(name='vod_s_t_start_kpi', flag='rtp_h264_4kp60')
    save_result(kpianalyze.kpifileobj_xlsx)
    kpianalyze.filter_abnormal_value(p_conf_rtp_4kP60_h264)
