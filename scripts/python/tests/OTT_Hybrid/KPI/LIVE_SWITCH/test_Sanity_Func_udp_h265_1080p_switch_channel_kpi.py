...
p_conf_udp_h265_1080p = config_yaml.get_note("kpi_config").get("live_s_t_switch_exit").get("udp_1080p")
...
for g_conf_device_id in devices:
    kpianalyze = KpiAnalyze(for_framework=True, adb=g_conf_device_id, config="/config/config.yaml",
                            ...
                            targetfile=None, kpifileobj=f"udp_h265_1080p_switch_exit_{g_conf_device_id}.txt",
                            kpifileobj_xlsx=f"udp_h265_1080p_switch_exit_{g_conf_device_id}.xlsx")
...
def multi_teardown():
    ...
    setprop()
    url = prepare("conf_udp_url", "udp", "conf_stream_name", "h265_1080P_3")
    push_xml()
    # set_url("udp://239.1.2.1:1234")
    # set_url("http://10.18.7.30/res/video/H264_1080P_PhilipsColorsofMiami_25M_25fps_5.5min.ts")
    adb.run_shell_cmd(f'am start -n {MULTIMEDIAPLAYER_TEST_APP_NAME}/.multiplay.MultiPlayActivity --esal url_list "{url};{url}" --ez is_loopplay true --ez is_amumediaplayer true --ez is_ts_mode true --ez IS_prefer_tunerhal false')
...
def test_kpi_result():
    ...
    kpianalyze.save_to_db(name='live_s_t_switch_channel_kpi', flag='udp_h265_1080p')
    save_result(kpianalyze.kpifileobj_xlsx)
    kpianalyze.filter_abnormal_value(p_conf_udp_h265_1080p)
