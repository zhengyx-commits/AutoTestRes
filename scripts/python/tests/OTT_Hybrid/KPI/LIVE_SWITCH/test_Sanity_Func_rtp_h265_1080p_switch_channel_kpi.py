...
p_conf_rtp_h265_1080p = config_yaml.get_note("kpi_config").get("live_s_t_switch_exit").get("rtp_1080p")
...
for g_conf_device_id in devices:
    kpianalyze = KpiAnalyze(for_framework=True, adb=g_conf_device_id, config="/config/config.yaml",
                            ...
                            targetfile=None, kpifileobj=f"rtp_h265_1080p_switch_exit_{g_conf_device_id}.txt",
                            kpifileobj_xlsx=f"rtp_h265_1080p_switch_exit_{g_conf_device_id}.xlsx")
...
def test_kpi_result():
    ...
    kpianalyze.save_to_db(name='live_s_t_switch_channel_kpi', flag='rtp_h265_1080p')
    save_result(kpianalyze.kpifileobj_xlsx)
    kpianalyze.filter_abnormal_value(p_conf_rtp_h265_1080p)
