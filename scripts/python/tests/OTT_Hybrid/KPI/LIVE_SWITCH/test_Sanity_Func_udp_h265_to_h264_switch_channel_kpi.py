...
p_conf_udp_h264_1080p = config_yaml.get_note("kpi_config").get("live_s_t_switch_exit").get("udp_1080p")
...
for g_conf_device_id in devices:
    kpianalyze = KpiAnalyze(for_framework=True, adb=g_conf_device_id, config="/config/config.yaml",
                                ...
                                targetfile=None, kpifileobj=f"udp_h264_1080p_switch_exit_{g_conf_device_id}.txt",
                                kpifileobj_xlsx=f"udp_h264_1080p_switch_exit_{g_conf_device_id}.xlsx")
    repeat_count = kpianalyze.repeat_count


@pytest.fixture(scope='module', autouse=True)
def multi_teardown():
    # get_filepath("Andy.Lau")
    setprop()
    url = prepare("conf_udp_url", "udp", "conf_stream_name", "h265_1080P_3")
    url1 = prepare("conf_udp_url", "udp", "conf_stream_name", "Andy_Lau")
    # url = "udp://239.1.2.1:1387"
    # set_url(f"{url}")
    push_xml()
...
def test_kpi_result():
    ...
    kpianalyze.save_to_db(name='live_s_t_switch_channel_kpi', flag='udp_h264_1080p')
    save_result(kpianalyze.kpifileobj_xlsx)
    kpianalyze.filter_abnormal_value(p_conf_udp_h264_1080p)
