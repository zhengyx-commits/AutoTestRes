...
p_conf_hls_h264_1080i = config_yaml.get_note("kpi_config").get("vod_s_t_kpi").get("hls_1080i_h264")
...
for g_conf_device_id in devices:
    kpianalyze = KpiAnalyze(for_framework=True, adb=g_conf_device_id, config="/config/config.yaml", kpi_name="vod_s_t_kpi",
                            targetfile=None, kpifileobj=f"hls_h264_1080i_start_{g_conf_device_id}.txt", kpifileobj_xlsx=f"hls_h264_1080i_start_{g_conf_device_id}.xlsx")
...
def multi_teardown():
    ...
    # set_url("http://10.18.19.108/files/mpeg2_1080I_30FPS/MPEG2-1080I-30fps.ts")
    setprop()
    set_url(f"http://{DEVICE_IP}/res/video/mpeg2_1080I_30FPS/MPEG2-1080I-30fps.ts")
    push_xml()
...
def test_kpi_result():
    ...
    kpianalyze.save_to_db(name='vod_s_t_start_kpi', flag='http_h264_1080i')
    save_result(kpianalyze.kpifileobj_xlsx)
    kpianalyze.filter_abnormal_value(p_conf_hls_h264_1080i)
