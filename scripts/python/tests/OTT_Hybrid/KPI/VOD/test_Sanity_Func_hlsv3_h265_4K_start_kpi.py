...
p_conf_hlsv3_h265_4k = config_yaml.get_note("kpi_config").get("vod_s_t_kpi").get("hlsv3_h265_4k")
...
for g_conf_device_id in devices:
    kpianalyze = KpiAnalyze(for_framework=True, adb=g_conf_device_id, config="/config/config.yaml", kpi_name="vod_s_t_kpi",
                            targetfile=None, kpifileobj=f"hlsv3_h265_4k_start_{g_conf_device_id}.txt", kpifileobj_xlsx=f"hlsv3_h265_4k_start_{g_conf_device_id}.xlsx")
...
def test_kpi_result():
    ...
    kpianalyze.save_to_db(name='vod_s_t_start_kpi', flag='hlsv3_h265_4k')
    save_result(kpianalyze.kpifileobj_xlsx)
    kpianalyze.filter_abnormal_value(p_conf_hlsv3_h265_4k)
