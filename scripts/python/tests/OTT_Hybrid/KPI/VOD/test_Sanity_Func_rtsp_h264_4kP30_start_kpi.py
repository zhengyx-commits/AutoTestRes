...
for g_conf_device_id in devices:
    kpianalyze = KpiAnalyze(for_framework=True, adb=g_conf_device_id, config="/config/config.yaml", kpi_name="vod_s_t_kpi",
                            targetfile=None, kpifileobj=f"rtsp_4kP30_h264_start_{g_conf_device_id}.txt", kpifileobj_xlsx=f"rtsp_4kP30_h264_start_{g_conf_device_id}.xlsx")
...
def multi_teardown():
    setprop()
    prepare("conf_rtsp_url", "rtsp_TS_H264_4K", "conf_stream_name", "h264_4K")
    push_xml()
...
def test_kpi_result():
    ...
    kpianalyze.save_to_db(name='vod_s_t_start_kpi', flag='rtsp_h264_4kp30')
